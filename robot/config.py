...
SUSPENSION_STATES = ("bottomed", "raised", "front_bottomed", "rear_bottomed")
if SUSPENSION_START_STATE not in SUSPENSION_STATES:
    raise ValueError("SUSPENSION_START_STATE must be one of: " + ", ".join(SUSPENSION_STATES))
if SUSPENSION_FAILSAFE_STATE not in SUSPENSION_STATES:
    raise ValueError("SUSPENSION_FAILSAFE_STATE must be one of: " + ", ".join(SUSPENSION_STATES))
...
STABILITY_POLL_HZ = env_float("STABILITY_POLL_HZ", "20.0")
if not 0.0 < STABILITY_POLL_HZ <= 1000.0:
    raise ValueError("STABILITY_POLL_HZ must be greater than 0 and at most 1000")
STABILITY_LOG_ENABLED = env_bool("STABILITY_LOG_ENABLED", "false")
STABILITY_LOG_DIR = os.environ.get("STABILITY_LOG_DIR", "logs/stability")
...
CONTROLLER_LEFT_TRIGGER_BUTTON = env_int("CONTROLLER_LEFT_TRIGGER_BUTTON", "312")
CONTROLLER_RIGHT_TRIGGER_BUTTON = env_int("CONTROLLER_RIGHT_TRIGGER_BUTTON", "313")
...
CONTROLLER_LEFT_TRIGGER_AXIS = env_int("CONTROLLER_LEFT_TRIGGER_AXIS", "2")
CONTROLLER_RIGHT_TRIGGER_AXIS = env_int("CONTROLLER_RIGHT_TRIGGER_AXIS", "5")
CONTROLLER_TRIGGER_THRESHOLD = env_float("CONTROLLER_TRIGGER_THRESHOLD", "0.5")
if not 0.0 < CONTROLLER_TRIGGER_THRESHOLD <= 1.0:
    raise ValueError("CONTROLLER_TRIGGER_THRESHOLD must be greater than 0 and at most 1")
...
CAPTURE_ROOT = os.environ.get("CAPTURE_ROOT", "datasets/captures")
CAPTURE_SESSION = os.environ.get("CAPTURE_SESSION", "")
CAPTURE_JPEG_QUALITY = env_int("CAPTURE_JPEG_QUALITY", "95")
if not 1 <= CAPTURE_JPEG_QUALITY <= 100:
    raise ValueError("CAPTURE_JPEG_QUALITY must be between 1 and 100")
CAPTURE_MIN_INTERVAL = env_float("CAPTURE_MIN_INTERVAL", "0.20")
if CAPTURE_MIN_INTERVAL < 0.0:
    raise ValueError("CAPTURE_MIN_INTERVAL must be non-negative")
